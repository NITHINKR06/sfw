...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get the parent directory (where examples are stored)
    const parentDir = path.resolve(projectRoot, "..");
    const normalizedPath = path.normalize(fullPath);

    // Security check: ensure the path is within the parent directory (one level up)
    // This allows accessing files in the SFW directory where examples are stored
